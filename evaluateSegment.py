...
for i,name in enumerate(files):
    ...
    plt.imsave(output_dir + '/' + imgName[0] + '.jpg',255*outputImg/5,cmap=cm.gray)
    gt = cv2.imread(groundtruth_dir + '/' + imgName[0] + '.jpg',0)
...
if verbose==1:
    ...
    t.add_row([' ',' ',' ',' ',' '])
    t.add_row(['All', str(round(np.sum(avg_error)/6,4)),str(round(np.sum(avg_precision)/6,4)),\
               str(round(np.sum(avg_recall)/6,4)),str(round(np.sum(avg_iou)/6,4))])
    print(t)
else:
